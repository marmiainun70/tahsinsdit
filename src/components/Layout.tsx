...
const navItems = [
  ...
  { to: "/class/1", icon: Users, label: "Data Siswa" },
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <motion.aside
        ...
        className="fixed left-0 top-0 h-full w-64 bg-sidebar z-30 flex flex-col lg:translate-x-0 lg:static lg:z-auto"
        style={{ transform: undefined }}
      >
        {/* Always visible on lg */}
        <div className="hidden lg:flex flex-col h-full w-64 bg-sidebar">
...
const SidebarContent = ({
  ...
  <div className="flex flex-col h-full">
    ...
    <nav className="flex-1 p-4 space-y-1">
      ...
      <div className="pt-3">
        <p className="text-sidebar-foreground/40 text-xs font-semibold uppercase tracking-wider px-3 mb-3">Kelas</p>
        {[1, 2, 3, 4, 5, 6].map(k => {
...
const Breadcrumb = ({ location }: { location: ReturnType<typeof useLocation> }) => {
  const path = location.pathname;
  if (path === "/") return <h2 className="font-semibold text-foreground">Dashboard</h2>;
  if (path === "/monitoring") return <h2 className="font-semibold text-foreground">Monitoring Seluruh Siswa</h2>;
  if (path === "/exam-list") return <h2 className="font-semibold text-foreground">Daftar Ujian</h2>;
  if (path.startsWith("/class/")) {
    const k = path.split("/")[2];
    return <h2 className="font-semibold text-foreground">Data Siswa — Kelas {k}</h2>;
  }
  if (path.startsWith("/student/")) return <h2 className="font-semibold text-foreground">Progres Siswa</h2>;
  if (path.startsWith("/exam/")) return <h2 className="font-semibold text-foreground">Ujian Kenaikan Level</h2>;
  return <h2 className="font-semibold text-foreground">Sistem Monitoring</h2>;
};
