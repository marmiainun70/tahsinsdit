...
const getStatusBadgeVariant = (status: "filled" | "empty") => status === "filled" ? "default" : "secondary";
...
const RecapReport = () => {
    ...
    return groups
      .map(g => {
        const filtered = g.rows.filter(r => {
          ...
          if (filterProgram !== "all" && r.bucket !== filterProgram) return false;
          return true;
        }).map((r, i) => ({ ...r, no: i + 1 }));
  ...
  const stats = useMemo(() => {
    ...
    const tahsinDasar = all.filter(r => r.bucket === "TD").length;
    
    const tl = all.filter(r => r.bucket === "TL");
    ...
    const tfz = all.filter(r => r.bucket === "TFZ");
              ...
              <div className="col-span-1 md:col-span-1 hidden md:block">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Melebihi Hari Efektif">Melebihi Hari</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="col-span-1 md:col-span-1 hidden md:block">
