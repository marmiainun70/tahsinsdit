...
const Examination = () => {
  ...
  const [form, setForm] = useState({
    ...
    kesalahanMakhraj: 0, kesalahanTajwid: 0, terhenti: 0, dibantuGuru: 0,
    catatan: "",
  });
  ...
  const nextLevel = getNextLevel(student.level as ReadingLevel);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const hasilUjian = lulus ? "Lulus" : "Tidak Lulus";
    await addExam.mutateAsync({
      student_id: student.id,
      level_diuji: student.level as ReadingLevel,
      kelancaran: form.kelancaran,
      makhraj: form.makhraj,
      tajwid: form.tajwid,
      adab: form.adab,
      kesalahan_makhraj: form.kesalahanMakhraj,
      kesalahan_tajwid: form.kesalahanTajwid,
      terhenti: form.terhenti,
      dibantu_guru: form.dibantuGuru,
  ...
  const NilaiInput = ({ label, field }: { label: string; field: keyof typeof form }) => (
    <div>
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <span className={`w-12 text-center text-sm font-bold px-2 py-1 rounded-lg ${(form[field] as number) >= 80 ? "bg-emerald-100 text-emerald-700" : (form[field] as number) >= 65 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-600"}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${LEVEL_COLORS[student.level as ReadingLevel]}`}>Ujian: {student.level}</span>
              {nextLevel && (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
            <h2 className="font-bold text-foreground mb-4 flex items-center gap-2">
              <span className="w-6 h-6 rounded-lg bg-gold flex items-center justify-center text-white text-xs font-bold">1</span>
              Penilaian Bacaan
            </h2>
            ...
            <div className="mt-5 p-4 bg-muted rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-2xl font-bold ${nilaiRata >= 80 ? "text-emerald-600" : nilaiRata >= 65 ? "text-yellow-600" : "text-red-500"}`}>{nilaiRata}</span>
              </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
            <h2 className="font-bold text-foreground mb-4 flex items-center gap-2">
              <span className="w-6 h-6 rounded-lg bg-gold flex items-center justify-center text-white text-xs font-bold">2</span>
              Catatan Kesalahan
            </h2>
            <div className="grid grid-cols-2 gap-4">
              {[["Kesalahan Makhraj", "kesalahanMakhraj"], ["Kesalahan Tajwid", "kesalahanTajwid"], ["Terhenti", "terhenti"], ["Dibantu Guru", "dibantuGuru"]].map(([label, key]) => (
                <div key={key}>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1.5">{label}</label>
                  <input type="number" min={0} value={form[key as keyof typeof form] as number} onChange={e => setForm(f => ({ ...f, [key]: parseInt(e.target.value) || 0 }))} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card focus:outline-none focus:ring-2 focus:ring-primary/30 text-sm" />
                </div>
              ))}
            </div>
          </div>
          ...
          <button type="submit" disabled={addExam.isPending || updateStudent.isPending} className="w-full py-3.5 bg-gradient-hero text-primary-foreground font-semibold rounded-xl hover:opacity-90 transition-opacity shadow-green text-base disabled:opacity-60 flex items-center justify-center gap-2">
            {(addExam.isPending || updateStudent.isPending) && <Loader2 className="w-5 h-5 animate-spin" />}
            Selesai & Tentukan Hasil Ujian
          </button>
        </form>
      ) : (
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-card rounded-2xl border border-border p-8 shadow-sm text-center">
          {hasil === "Lulus" ? (
            <>
            ...
            <>
              ...
              <p className="text-muted-foreground mb-4"><span className="font-semibold text-foreground">{student.nama}</span> belum memenuhi syarat kenaikan level.</p>
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
                <p className="text-yellow-700 text-sm">Nilai rata-rata: <span className="font-bold">{nilaiRata}</span> (minimal 80)</p>
              </div>
            </>
          )}
          {form.catatan && <p className="text-sm text-muted-foreground italic bg-muted rounded-xl px-4 py-3 mb-6">"{form.catatan}"</p>}
          <div className="flex gap-3 justify-center">
            <button onClick={() => { setSubmitted(false); setHasil(null); }} className="px-5 py-2.5 border border-border rounded-xl text-sm font-medium hover:bg-muted transition-colors">Ujian Ulang</button>
            <Link to={`/student/${student.id}`}><button className="px-5 py-2.5 bg-gradient-hero text-primary-foreground rounded-xl text-sm font-medium hover:opacity-90 transition-opacity shadow-green">Kembali ke Profil</button></Link>
          </div>
        </motion.div>
      )}
    </div>
