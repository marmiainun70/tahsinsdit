...
import {
  Plus, FileText, Loader2, Trash2, CheckCircle2, XCircle, Filter
} from "lucide-react";
...
const MonthlyReport = () => {
  ...
  const programType = selectedStudent
    ...
    : "iqra";
  const iqraLevel = selectedStudent && IQRO_LEVELS.includes(selectedStudent.level)
            ...
            <div className="space-y-4">
              ...
              {selectedStudent && (
                <div className="p-3 bg-muted rounded-xl space-y-1 text-sm">
                  <p><span className="font-medium">Program:</span> {programType === "iqra" ? "Tahsin Dasar (Iqra)" : "Tahsin Lanjutan"}</p>
                  {iqraLevel && <p><span className="font-medium">Level Iqra:</span> {iqraLevel}</p>}
                      ...
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className="text-xs">
                            {r.program_type === "iqra" ? `Iqra${r.iqra_level ? ` (${r.iqra_level})` : ""}` : "Tahsin"}
                          </Badge>
                        </TableCell>
